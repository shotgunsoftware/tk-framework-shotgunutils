...
def _get_thumbnail_path(url, bundle):
    ...
    # move files around. 
    old_path = _get_thumbnail_path_old(url, bundle)
...
class _RetrieverThread(QtCore.QThread):
    ...
    def _get_shotgun_connection(self):
        if self._sg is None:
            ...
            self._sg = tank.util.shotgun.create_sg_connection()

            # set the maximum timeout for this connection for fluency
            self._sg.config.timeout_secs = CONNECTION_TIMEOUT_SECS
        
        return self._sg
    # @shotgun_connection.setter
    def _set_shotgun_connection(self, value):
        self._sg = value
    shotgun_connection = property(_get_shotgun_connection, _set_shotgun_connection)
        ...
        while self._process_queue:
            ...
            try:
                ...
                if item_type == _RetrieverThread._SG_FIND_QUERY:
                    ...
                    sg = self.shotgun_connection.find(item_to_process["entity_type"],
                    ...
                    path_to_cached_thumb = _get_thumbnail_path(url, self._bundle)
...
class ShotgunDataRetrieverBase(QtCore.QObject):
    ...
    def download_thumbnail(url, bundle):
        ...
        path_to_cached_thumb = _get_thumbnail_path(url, bundle)
...
class SynchrounousShotgunDataRetriever(ShotgunDataRetrieverBase):
    
    def __init__(self, parent=None):
        """
        """
        ShotgunDataRetrieverBase.__init__(self, parent)
        self._bundle = tank.platform.current_bundle()
    ...
    def execute_find(self, entity_type, filters, fields, order = None):
        """
        """
        sg_res = self._sg.find(entity_type, filters, fields, order)
        return sg_res
    ...
    def request_thumbnail(self, url, entity_type, entity_id, field):
        ...
        if not os.path.exists(path_to_cached_thumb):
            # download the actual thumbnail. Because of S3, the url
            # has most likely expired, so need to re-fetch it via a sg find
            sg_data = self._sg.find_one(entity_type, [["id", "is", entity_id]], [field])
            if sg_data and sg_data.get(field):
                url = sg_data[field]
                self._bundle.ensure_folder_exists(os.path.dirname(path_to_cached_thumb))
                tank.util.download_url(self._sg, url, path_to_cached_thumb)
                # modify the permissions of the file so it's writeable by others
                old_umask = os.umask(0)
                try:
                    os.chmod(path_to_cached_thumb, 0666)
...
class ShotgunDataRetriever(ShotgunDataRetrieverBase):
    ...
    def set_shotgun_connection(self, sg):
        self._retriever_thread.shotgun_connection = value

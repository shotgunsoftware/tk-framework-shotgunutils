...
class ShotgunFindDataHandler(ShotgunDataHandler):
    ...
    def update_find_data(self, sg_data, hierarchy):
        ...
        new_cache = {self.CACHE_CHILDREN: {}, self.CACHE_BY_UID: {}}
            ...
            for field_name in hierarchy:
                ...
                if on_leaf_level:
                    ...
                    if unique_field_value not in self._cache[self.CACHE_BY_UID]:
                        ...
                        current_record = self._cache[self.CACHE_BY_UID][unique_field_value]["sg_data"]
        ...
        self._log_debug("Shotgun data received and processed. %d diffs from in-memory cache." % len(diff_list))
        return diff_list
    ...
    def __generate_unique_key(self, field, sg_data):
        ...
        if isinstance(value, dict) and "id" in value and "type" in value:
            return "%s_%s_%s" % (field, value["type"], value["id"])

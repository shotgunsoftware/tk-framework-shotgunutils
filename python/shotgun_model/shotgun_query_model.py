                ...
                if item["mode"] == self._data_handler.ADDED:
                    ...
                    if parent_model_item:
                        ...
                        if not self.canFetchMore(parent_model_item.index()):
                            ...
                            # Double check that the item we pulled from SG
                            # was not already added by a fetchMore on the model.
                            # If it is the case, we just update the existing item
                            # even, if in theory, it should already be up to date.
                            model_item = self._get_item_by_unique_id(
                                data_item.unique_id
                            )
                            if model_item:
                                self._log_debug(
                                    "Updating existing model item %s for create" % model_item
                                )
                                print(
                                    "Updating existing model item %s for create" % model_item
                                )
